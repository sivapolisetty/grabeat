-- PRODUCTION DATABASE FIX: Add missing columns for simplified order flow
-- Run this against the production Supabase database to fix the missing columns

-- Add new fields for verification system (if they don't exist)
DO $$ 
BEGIN
    -- Add verification_code column
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'orders' AND column_name = 'verification_code') THEN
        ALTER TABLE orders ADD COLUMN verification_code VARCHAR(6) NOT NULL DEFAULT '';
    END IF;
    
    -- Add qr_data column
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'orders' AND column_name = 'qr_data') THEN
        ALTER TABLE orders ADD COLUMN qr_data TEXT;
    END IF;
...
-- First, update existing orders to use simplified statuses BEFORE adding constraints
UPDATE orders 
SET status = CASE 
    WHEN status IN ('pending', 'preparing', 'ready') THEN 'confirmed'
    WHEN status = 'delivered' THEN 'completed'
    ELSE status
END
WHERE status NOT IN ('confirmed', 'completed', 'cancelled');

-- Now update status constraints to only allow simplified states
ALTER TABLE orders 
...
BEGIN
  ...
  IF NEW.status = 'confirmed' AND (OLD IS NULL OR OLD.confirmed_at IS NULL) THEN
...
-- Update existing orders to have verification codes (status already updated above)
UPDATE orders 
SET 
...
WHERE table_name = 'orders' 
  AND column_name IN ('verification_code', 'qr_data', 'confirmed_at', 'completed_at')
