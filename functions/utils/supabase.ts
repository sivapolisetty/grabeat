...
export function createServiceRoleClient(env: Env) {
  // Use local Supabase for database operations (NoenCircles pattern)
  const url = env.LOCAL_SUPABASE_URL || env.SUPABASE_URL;
  const key = env.LOCAL_SUPABASE_SERVICE_ROLE_KEY || env.SUPABASE_SERVICE_ROLE_KEY!;
